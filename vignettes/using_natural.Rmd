...
The tuning parameter path can be specified via argument `lambda`. If not provided, the algorithm will automatically generate a path of lambda of length `nlam`.
```{r, fig.height = 5, fig.width = 5, fig.align='center'}
nl <- nlasso_path(x = sim$x, y = sim$y[, 1])
plot(nl$lambda, nl$sig_naive_path, type = "l", col = "black", lwd = 2, main = NULL, ylab = "estimated error s.d.", xlab = "lambda")
lines(nl$lambda, nl$sig_df_path, col = "red", lwd = 2)
lines(nl$lambda, nl$sig_obj_path, col = "blue", lwd = 2)
abline(h = sim$sigma, lwd = 1, col = "gray")
legend("bottomright", legend = c("naive", "df", "natural", "truth"), lwd = c(2, 2, 2, 1), col = c("black", "red", "blue", "gray"))
```
...
<!--
We consider a small simulation here to compare the perform the three methods:
```{r, fig.height = 5, fig.width = 5, fig.align='center'}
err_mat <- matrix(NA, nrow = nsim, ncol = 3)
colnames(err_mat) <- c("natural", "naive", "df")
for(i in seq(nsim)){
  curr <- nlasso_cv(x = sim$x, y = sim$y[, i])
  err_mat[i, 1] <- (curr$sig_obj / sim$sigma - 1)^2
  err_mat[i, 2] <- (curr$sig_naive / sim$sigma - 1)^2
  err_mat[i, 3] <- (curr$sig_df / sim$sigma - 1)^2
}
boxplot(err_mat)
```
-->
...
Organic lasso is a companion method to the natural lasso. The main novelty is that the choice of tuning parameter is *pivotal*, in that it does not depend on any unknown parameter. The organic lasso estimate of error variance is defined as
$$
